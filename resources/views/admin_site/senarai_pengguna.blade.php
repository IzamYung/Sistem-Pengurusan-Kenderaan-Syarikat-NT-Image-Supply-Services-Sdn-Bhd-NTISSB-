...
<div class="max-w-6xl mx-auto mt-10 mb-24 px-4 sm:px-6">
    ...
    <div class="mb-12 text-center">
        ...
        <p class="text-gray-500 font-medium">
            {{ $tambahMode || $editUser ? 'Sila lengkapkan maklumat pengguna di bawah.' : 'Uruskan maklumat dan akses kakitangan dalam sistem secara sistematik.' }}
        </p>
    </div>
        ...
        <div class="max-w-4xl mx-auto bg-white shadow-[0_20px_50px_rgba(0,0,0,0.05)] rounded-[2.5rem] overflow-hidden border border-gray-100">
            <div class="bg-gradient-to-r from-[#1e3a8a] to-blue-800 px-8 py-6 text-center">
                <h2 class="text-white font-bold text-xl tracking-wide uppercase">
                    {{ $tambahMode ? 'Daftar Pengguna Baharu' : 'Kemaskini Profil Pengguna' }}
                </h2>
            </div>
            ...
            <form method="POST" 
                  action="{{ $tambahMode ? route('admin_site.tambah_pengguna.store') : route('admin_site.tambah_pengguna.update', $editUser->id_pekerja) }}" 
                            ...
                            <div class="relative group">
                                <div class="w-32 h-32 rounded-2xl overflow-hidden bg-white shadow-md border-4 border-white">
                                    <img id="image-preview" 
                                         src="{{ asset($editUser->gambar_profil ?? 'images/profile_picture/default-profile.png') }}" 
                ...
                <div class="mt-12 flex flex-col sm:flex-row gap-4 items-center justify-center border-t border-gray-100 pt-10">
                    <a href="{{ route('admin_site.senarai_pengguna') }}" 
        ...
        <div class="max-w-6xl mx-auto">
            ...
            <div class="space-y-4" id="userContainer">
                @forelse($users as $user)
                    <div class="user-card group bg-white p-4 md:p-6 rounded-[2rem] shadow-[0_10px_30px_rgba(0,0,0,0.02)] border border-gray-100 hover:border-blue-400 hover:shadow-[0_20px_40px_rgba(30,58,138,0.08)] transition-all duration-300 flex items-center"
                        ...
                        <a href="{{ route('admin_site.tambah_pengguna.edit', $user->id_pekerja) }}" 
                           class="flex-1 grid grid-cols-12 gap-4 px-6 items-center">
                            ...
                            <div class="hidden md:block col-span-3">
                                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Jawatan</p>
                                <p class="text-sm font-bold text-gray-700 truncate">{{ $user->jawatan ?? '-' }}</p>
                            </div>
                            <div class="hidden md:block col-span-5">
                                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Email / Telefon</p>
                                <p class="text-sm font-medium text-gray-500 truncate">{{ $user->email }}</p>
                                <p class="text-[10px] font-bold text-blue-500">{{ $user->no_tel ?? '-' }}</p>
                            </div>
                        </a>
                ...
                <p class="text-gray-400 hidden py-10 text-center font-bold italic" id="noMatch">
            ...
            <div class="mt-12 flex justify-center">
                <nav class="inline-flex gap-2 p-2 bg-white rounded-2xl shadow-sm border border-gray-100" role="navigation">
                    ...
                    @else
                        <a href="{{ $users->previousPageUrl() }}" class="px-4 py-2 text-blue-600 hover:bg-blue-50 rounded-xl transition-all font-black">&larr;</a>
                    @endif
