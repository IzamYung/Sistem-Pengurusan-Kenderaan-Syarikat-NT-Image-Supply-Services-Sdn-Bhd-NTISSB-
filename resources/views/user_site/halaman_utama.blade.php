...
<div class="max-w-5xl 2xl:max-w-6xl mx-auto mt-6 sm:mt-10 mb-16 sm:mb-20 px-3 sm:px-4 lg:px-0">
    <div class="bg-white rounded-[2.5rem] sm:rounded-[3rem] shadow-2xl overflow-hidden border border-gray-100">
        <div class="bg-[#1e3a8a] py-8 sm:py-12 px-6 sm:px-8 relative overflow-hidden">
            <div class="absolute top-0 right-0 w-48 sm:w-64 h-48 sm:h-64 bg-white/5 rounded-full -mr-24 sm:-mr-32 -mt-24 sm:-mt-32"></div>
            <h2 class="text-2xl sm:text-3xl xl:text-4xl font-black text-white text-center tracking-[0.15em] sm:tracking-[0.2em] uppercase relative z-10">
                Borang Permohonan
            </h2>
            <p class="text-blue-200 text-[10px] sm:text-xs mt-2 sm:mt-3 text-center uppercase tracking-[0.25em] sm:tracking-[0.3em] font-bold relative z-10">
                Sistem Pengurusan Kenderaan
            </p>
        </div>

        <form method="POST" action="{{ route('user_site.permohonan.store') }}" enctype="multipart/form-data"
              class="p-6 sm:p-8 md:p-14 space-y-8 sm:space-y-10">
            @csrf

            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 bg-gray-50 p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border border-gray-100">
                <div class="space-y-1">
                    <label class="block text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest">No. Pendaftaran</label>
                    <p class="text-base sm:text-lg font-bold text-[#1e3a8a]">{{ $kenderaan->no_pendaftaran ?? '-' }}</p>
                </div>
                <div class="space-y-1">
                    <label class="block text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest">Model Kenderaan</label>
                    <p class="text-base sm:text-lg font-bold text-[#1e3a8a]">{{ $kenderaan->model ?? '-' }}</p>
                </div>
                <div class="space-y-1">
                    <label class="block text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest">Nama Pemohon</label>
                    <p class="text-base sm:text-lg font-bold text-[#1e3a8a]">{{ $user->nama ?? '-' }}</p>
                </div>
            </div>

            <input type="hidden" name="no_pendaftaran" value="{{ $kenderaan->no_pendaftaran ?? '' }}">

            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-6 sm:gap-y-8">
                <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
                    <div>
                        <label class="block text-[10px] sm:text-xs font-black text-gray-700 mb-2 sm:mb-3 ml-2 uppercase tracking-widest">
                        ...
                        <input type="text" name="tarikh_pelepasan" id="tarikhPelepasan"
                               ...
                               class="w-full border-2 border-gray-100 bg-gray-50 rounded-xl sm:rounded-2xl px-4 sm:px-6 py-3 sm:py-4 focus:ring-4 focus:ring-blue-50 focus:border-[#1e3a8a] transition-all outline-none font-bold text-gray-700"
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="lokasi" required value="{{ old('lokasi') }}"
                           class="w-full border-2 border-gray-100 bg-gray-50 rounded-xl sm:rounded-2xl px-4 sm:px-6 py-3 sm:py-4 focus:ring-4 focus:ring-blue-50 focus:border-[#1e3a8a] focus:bg-white transition-all outline-none font-bold text-gray-700"
                           placeholder="Contoh: Mesyuarat Tapak di Melaka"/>
                </div>

                <div>
                    <label class="block text-[10px] sm:text-xs font-black text-gray-700 mb-2 sm:mb-3 ml-2 uppercase tracking-widest">
                        Bilangan Penumpang <span class="text-red-500">*</span>
                    </label>
                ...
                <div>
                    <label class="block text-[10px] sm:text-xs font-black text-gray-700 mb-2 sm:mb-3 ml-2 uppercase tracking-widest">
                        Kod Projek <span class="text-red-500">*</span>
                    </label>
                ...
                <div>
                    <label class="block text-[10px] sm:text-xs font-black text-gray-700 mb-2 sm:mb-3 ml-2 uppercase tracking-widest">
                        Hak Milik <span class="text-red-500">*</span>
                    </label>
                ...
                <div>
                    ...
                    <div class="upload-container">
                        ...
                        <label for="lampiranInput"
                               class="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 border-2 border-dashed border-gray-200 rounded-2xl cursor-pointer hover:border-[#1e3a8a] hover:bg-white transition-all group">
                            <div class="bg-blue-50 group-hover:bg-[#1e3a8a] p-2.5 sm:p-3 rounded-xl transition-all">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-[#1e3a8a] group-hover:text-white"
                            ...
                            <div>
                                <p class="text-[10px] sm:text-[11px] font-black text-[#1e3a8a] uppercase tracking-widest">
                                    Muat Naik Fail
                                </p>
                                <p class="text-[8px] sm:text-[9px] text-gray-400 font-bold uppercase">
            ...
            <div class="pt-8 sm:pt-10">
                <button type="submit"
                        class="w-full py-4 sm:py-6 bg-[#1e3a8a] hover:bg-blue-800 text-white rounded-2xl sm:rounded-[2rem] font-black text-xs sm:text-sm uppercase tracking-[0.25em] sm:tracking-[0.3em] shadow-xl shadow-blue-100 transform active:scale-[0.98] transition-all">
                    Hantar Permohonan
                </button>
            </div>
        </form>
    ...
    <div class="flex flex-col gap-8" id="vehicleList">
        @forelse ($kenderaan ?? [] as $car)
            ...
            >
                ...
                <div class="p-8 md:p-10 flex-1 flex flex-col relative">
                    ...
                    <div class="absolute bottom-8 right-8 hidden lg:block">
                        <div class="w-12 h-12 rounded-2xl bg-gray-50 flex items-center justify-center text-gray-300 transition-all duration-300
                            group-hover:bg-[#1e3a8a] group-hover:text-white group-hover:rotate-[-45deg] shadow-sm">
