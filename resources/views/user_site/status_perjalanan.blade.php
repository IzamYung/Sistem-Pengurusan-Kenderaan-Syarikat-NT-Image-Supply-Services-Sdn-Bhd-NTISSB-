...
<div class="max-w-5xl mx-auto mt-10 mb-20 px-4">
    ...
    @forelse($senarai as $item)
        <form method="POST"
              ...
              class="bg-white shadow-xl rounded-3xl overflow-hidden border border-gray-100 transition-all hover:shadow-2xl"
              enctype="multipart/form-data">
            @csrf

            {{-- Hidden ID - JANGAN BUANG --}}
                    ...
                    <div class="lg:col-span-4 space-y-6">
                        ...
                        <div class="p-1">
                            ...
                            <textarea name="ulasan"
                                      class="w-full border border-gray-200 rounded-2xl px-4 py-3 text-sm focus:ring-2 focus:ring-blue-500 transition-all outline-none"
                                      rows="4" 
                                      placeholder="Tulis ulasan perjalanan di sini...">{{ $item->ulasan }}</textarea>
                        </div>
                    </div>

                    {{-- UPLOAD SPEEDOMETER (RIGHT) --}}
                    <div class="lg:col-span-8">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="relative group border-2 border-dashed border-gray-200 rounded-2xl p-2 hover:border-blue-400 transition-colors bg-gray-50/50">
                                    <div class="aspect-video w-full rounded-xl overflow-hidden bg-gray-200 relative mb-2">
                                        <img id="preview-sebelum-{{ $item->id_permohonan }}"
                                             src="{{ $item->speedometer_sebelum ? asset('storage/'.$item->speedometer_sebelum) : 'https://placehold.co/600x400?text=Sila+Muat+Naik' }}"
                                             class="speedometer-preview w-full h-full object-cover cursor-pointer"
                                             data-modal-img="{{ $item->speedometer_sebelum ? asset('storage/'.$item->speedometer_sebelum) : '' }}">
                                    </div>
                                    
                                    <input type="file"
                                           name="speedometer_sebelum"
                                           accept=".jpg,.jpeg,.png,.webp"
                                           class="w-full text-xs text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer">
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="relative group border-2 border-dashed border-gray-200 rounded-2xl p-2 hover:border-blue-400 transition-colors bg-gray-50/50">
                                    <div class="aspect-video w-full rounded-xl overflow-hidden bg-gray-200 relative mb-2">
                                        <img id="preview-selepas-{{ $item->id_permohonan }}"
                                             src="{{ $item->speedometer_selepas ? asset('storage/'.$item->speedometer_selepas) : 'https://placehold.co/600x400?text=Sila+Muat+Naik' }}"
                                             class="speedometer-preview w-full h-full object-cover cursor-pointer"
                                             data-modal-img="{{ $item->speedometer_selepas ? asset('storage/'.$item->speedometer_selepas) : '' }}">
                                    </div>
            ...
            <div class="px-8 py-5 bg-gray-50 border-t border-gray-100 flex justify-end">
                <button type="submit"
                        class="px-10 py-3 bg-[#1e3a8a] text-white rounded-xl hover:bg-black font-extrabold transition-all shadow-lg hover:shadow-xl active:scale-95 flex items-center gap-2 uppercase tracking-wider">
